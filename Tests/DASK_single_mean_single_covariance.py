    ...
    with Client(address='localhost:8001') as client:
        for n in range(5):
            ...
            for i in range(batch_size):
                convolved_means['i'] = []
                convolved_covariances['i'] = []
                pre_relu_samples['i'] = []
                post_relu_samples['i'] = []
                end_means['i'] = []

                for j in range(total_kernels):
                    convolved_covariances['i'].append(
                        cov_mult(convmatrix[i, :, :], cov[j, :, :])
                    )
                    convolved_means['i'].append(
                        da.matmul(kernels[j, :], convmatrix[i, :, :])
                    )
                for (mean, covariance) in zip(convolved_means['i'], convolved_covariances['i']):
                    pre_relu_samples['i'].append(
                        mvn_random_DASK(mean, covariance, total_samples, input_size ** 2)
                    )
                for samples in pre_relu_samples['i']:
                    post_relu_samples['i'].append(
                        relu(samples)
                    )
                for samples in post_relu_samples['i']:
                    end_means['i'].append(
                        samples.mean(axis=1)
                    )
            ...
            # combined_list = [value for key, value in out_image_list]
